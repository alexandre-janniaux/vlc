...
struct SurfaceTextureHandle {
    ...
    /* Real underyling SurfaceTexture */
    jobject         jtexture;
    ASurfaceTexture *texture;

    /* We might store a Surface object if the ASurfaceTexture
...
success:
    ...
    if (p_awh->b_has_ndk_ast_api)
    {
        ...
        handle->jsurface = p_awh->ndk_ast_api.pf_anwToSurface(p_env, handle->surface.window);
    }
    else
    {
        ...
        handle->surface.window = p_awh->pf_winFromSurface(p_env, handle->jsurface);
...
ANativeWindow *
vlc_asurfacetexture_GetANativeWindow(struct vlc_asurfacetexture *st)
{
    struct SurfaceTextureHandle *handle =
        container_of(st, struct SurfaceTextureHandle, surface);
    return handle->surface.window;
}

jobject
vlc_asurfacetexture_GetSurface(struct vlc_asurfacetexture *st)
{
    struct SurfaceTextureHandle *handle =
        container_of(st, struct SurfaceTextureHandle, surface);
