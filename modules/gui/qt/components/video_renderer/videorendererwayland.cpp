...
void VideoRendererWayland::setVoutWindow( vout_window_t *window )
{
    VideoRenderer::setVoutWindow( window );
    ...
    window->display.wl = static_cast<wl_display *>(
    ...
    wl_registry *registry = wl_display_get_registry( window->display.wl );
    wl_registry_add_listener( registry, &registry_cbs, this );
    wl_display_roundtrip( window->display.wl );
    ...
    m_surface =
    window->handle.wl = wl_compositor_create_surface( m_compositor );
    ...
    wl_surface_set_input_region( window->handle.wl, region );
...
void VideoRendererWayland::registerVideoCallbacks( vlc_player_t * )
{
    // nothing to do
}
