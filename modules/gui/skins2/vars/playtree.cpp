...
Playtree::Playtree( intf_thread_t *pIntf )
    : VarTree( pIntf ), m_playlist(vlc_intf_GetMainPlaylist(pIntf))
{
...
void Playtree::delSelected()
{
    for( Iterator it = m_children.begin(); it != m_children.end(); )
    {
        if( it->isSelected() && !it->isReadonly() )
        {
            ...
            auto index = vlc_playlist_IndexOfId(m_playlist, it->getId());
            vlc_playlist_RemoveOne(m_playlist, index);
...
void Playtree::action( VarTree *pElem )
{
    ...
    auto index = vlc_playlist_IndexOfId(m_playlist, pElem->getId());
    if (index > 0)
        vlc_playlist_PlayAt(m_playlist, index);
