...
static int Open( vlc_object_t *p_this )
{
    ...
    if( !p_sys )
        return VLC_ENOMEM;
    p_sys->i_nb = var_CreateGetInteger( p_filter->obj.members.parent,
                                        "norm-buff-size" );
    p_sys->f_max = var_CreateGetFloat( p_filter->obj.members.parent,
                                       "norm-max-level" );
...
static block_t *DoWork( filter_t *p_filter, block_t *p_in_buf )
{
    ...
    for( i_chan = 0; i_chan < i_channels; i_chan++ )
    {
        ...
        /* Seuil arbitraire */
        p_sys->f_max = var_GetFloat( p_filter->obj.members.parent,
                                     "norm-max-level" );
