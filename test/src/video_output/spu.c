...
//static subpicture_t* create_square(int x, int y, int width, int height)
//{
//    video_format_t fmt;
...
//
//    subpicture_t *subpic = subpicture_NewFromPicture();
//    return subpic;
//}
                          ...
                          const video_format_t* fmt_dst)
{
    ...
    assert(output1->p_region->fmt.i_sar_num == output1->p_region->fmt.i_sar_den);
    ...
    int i_sar_num1 = output1->p_region->fmt.i_sar_num;
    int i_sar_den1 = output1->p_region->fmt.i_sar_den;
    int i_x2 = output2->p_region->i_x;
    int i_y2 = output2->p_region->i_y;
    int i_sar_num2 = output2->p_region->fmt.i_sar_num;
    int i_sar_den2 = output2->p_region->fmt.i_sar_den;

    msg_Err(spu1, "x1=%d, y1=%d, width1=%d, height1=%d, sar1=%d/%d",
            i_x1, i_y1, width1, height1, i_sar_num1, i_sar_den1);
    msg_Err(spu2, "x2=%d, y2=%d, width2=%d, height2=%d, sar2=%d/%d",
            i_x2, i_y2, width2, height2, i_sar_num2, i_sar_den2);
    ...
    /* Width and height depends on aspect ratio, check that the renderer
     * will correctly handle this size change. */
    struct vlc_subpicture_place place1, place2;
    vlc_subpicture_region_Place(&place1, output1->p_region);
    vlc_subpicture_region_Place(&place2, output2->p_region);
    msg_Info(spu1, "left=%f, right=%f, top=%f, bottom=%f",
            place1.left, place1.right, place1.top, place1.bottom);
    msg_Info(spu2, "left=%f, right=%f, top=%f, bottom=%f",
            place2.left, place2.right, place2.top, place2.bottom);
    // TODO: relative diff
    assert(0.001 > fabs(place1.left   - place2.left));
    assert(0.001 > fabs(place1.right  - place2.right));
    assert(0.001 > fabs(place1.top    - place2.top));
    assert(0.001 > fabs(place1.bottom - place2.bottom));
...
int main(int argc, char **argv)
{
    ...
    struct vlc_subpicture_place output;
    struct vlc_subpicture_place expected_output =
    {
        .left = -.5f,   .right  = .5f,
        .top  = -.5f,   .bottom = .5f
    };
    ...
    subpicture_region_t region =
    {
        ...
        .fmt = {
            .i_sar_num = 1,     .i_sar_den = 1,
            .i_visible_width = 800,
            .i_visible_height = 600,
        }
    };

    vlc_subpicture_region_Place(&output, &region);
    fprintf(stderr, "output: left=%f, right=%f, top=%f, bottom=%f\n",
