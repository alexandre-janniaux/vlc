...
static bool PlayItem( playlist_t *p_playlist, playlist_item_t *p_item )
{
    ...
    libvlc_MetadataCancel( p_playlist->obj.members.libvlc, p_item );
    ...
    if( !b_has_art || strncmp( psz_arturl, "attachment://", 13 ) )
    {
        ...
        libvlc_ArtRequest( p_playlist->obj.members.libvlc, p_input, META_REQUEST_OPTION_NONE );
    }
    free( psz_arturl );
...
static void *Thread ( void *data )
{
    ...
    while( !p_sys->killed )
    {
        ...
        if( played && var_InheritBool( p_playlist, "play-and-exit" ) )
        {
            ...
            libvlc_Quit( p_playlist->obj.members.libvlc );
        }
