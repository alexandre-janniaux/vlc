                       ...
                       void *p_data )
{
    ...
    vlm_t *p_vlm = libvlc_priv( p_input->obj.members.libvlc )->p_vlm;
    assert( p_vlm );
...
vlm_t *vlm_New( libvlc_int_t *libvlc, const char *psz_vlmconf )
{
    ...
    p_vlm = vlc_custom_create( p_this->obj.members.libvlc, sizeof( *p_vlm ),
                               "vlm daemon" );
    if( !p_vlm )
    {
        vlc_mutex_unlock( &vlm_mutex );
...
void vlm_Delete( vlm_t *p_vlm )
{
    ...
    if( --p_vlm->users == 0 )
        assert( libvlc_priv(p_vlm->obj.members.libvlc)->p_vlm == p_vlm );
    else
        p_vlm = NULL;
    ...
    libvlc_priv(p_vlm->obj.members.libvlc)->p_vlm = NULL;
    vlc_mutex_unlock( &vlm_mutex );
