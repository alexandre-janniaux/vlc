...
static void QuaternionToEuler(float *yaw, float *pitch, float *roll, const float *q)
{
    ...
    /* CONVENTION:
     * w, x, y, z lexical order on multiplication and addition */
    ...
    float V_21 = 2 * (gw * gz + gx * gy);
    float V_23 = 2 * (gw * gx + gy * gz);

    /* Values with a minus sign. */
    float V_12 = 2 * (gw * gz - gx * gy);
    ...
    if (V_32 > 0.990 * unit)
    {
        ...
        // DOESNT WORK WITH ROLL=90
        *yaw = -atan2( V_13, V_12 );
        *pitch = M_PI_2;
        *roll = 0;
    }
    else if (V_32 < -0.999 * unit)
    {
    ...
    else
    {
        ...
        *roll  = -atan2( V_12, V_22 );
        *pitch = -asin( V_32 );
        *pitch = atan2( -V_32, sqrt( V_33 * V_33 + V_31 * V_31 ) );
    }
}
